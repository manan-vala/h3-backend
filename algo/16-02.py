...
class OptimizedRoute:
    ...
    def set_requests_map(self, requests_map):
        self.requests_map = requests_map
...
class Solution:
    ...
    def shallow_copy(self) -> 'Solution':
        """Deep-copy routes to prevent destroy operators from corrupting the original."""
        s = Solution()
        s.routes = {vid: r.copy() for vid, r in self.routes.items()}
        s.unassigned = self.unassigned.copy()
        s.iteration_found = self.iteration_found
    ...
    def _validate_route_full(self, route, requests_map):
        ...
        if route.get_max_load() > route.vehicle.capacity:
            return False, f"CAPACITY"
        pickup_times, drop_times = {}, {}
...
class EnhancedALNSSolver:
        ...
        for i in range(n_orig + 2):
            ...
            if i == drop_idx:
                if not new_legs:
                    return None
                prev_loc = prev_loc_from_legs(new_legs)
                tt = prev_loc.distance_to(req.drop_loc) / route.vehicle.speed_kmph / 24
                arrival = curr_time + tt
                departure = arrival
                if departure > req.latest_drop_hard + 1e-9:
                    return None
                curr_load -= 1
            ...
            if orig_i < n_orig:
                ...
                tt = prev_loc.distance_to(curr_loc) / route.vehicle.speed_kmph / 24
                arrival = curr_time + tt
                if leg.is_pickup:
                    departure = max(arrival, lr.earliest_pickup)
                else:
                    departure = arrival
                    if departure > lr.latest_drop_hard + 1e-9:
                        return None
                curr_load += 1 if leg.is_pickup else -1
                if curr_load > route.vehicle.capacity or curr_load < 0:
                    return None
    ...
    def _find_best_insertion(self, route, req):
        """
        Find the cheapest feasible (p_idx, d_idx) insertion.
        Pruning rules to keep this O(n) instead of O(n²):
          - Skip pickup positions where a previous leg's departure
            is already past req.latest_drop_hard.
          - Limit drop to at most MAX_GAP positions after pickup.
            (nearby drop = less detour; far drops rarely pay off)
        """
        MAX_GAP = 2   # max legs between pickup and drop insertion
        n = len(route.legs)
        best_score = float('inf')
        best_positions = None

        for p_idx in range(n + 1):
            # Prune: if the leg BEFORE p_idx departed after req's deadline
            # there's no point inserting pickup here.
            if p_idx > 0:
                prev_dep = route.legs[p_idx - 1].departure_time
                if prev_dep > req.latest_drop_hard + 1e-9:
                    break  # all later p_idx will be even worse

            for d_idx in range(p_idx + 1,
                               min(p_idx + 1 + MAX_GAP, n + 2) + 1):
                trial = self._insert_request(route.copy(), req, p_idx, d_idx)
                if trial:
                    valid, _ = self._validate_route(trial)
                    if valid:
                        cost = (trial.total_distance *
                                trial.vehicle.cost_per_km)
                        if cost < best_score:
                            best_score = cost
    ...
    def _rebuild_route(self, vehicle, legs):
        ...
        for leg in legs:
            req = self.requests[leg.request_id]
            curr_loc = req.pickup_loc if leg.is_pickup else req.drop_loc
            tt = prev_loc.distance_to(curr_loc) / vehicle.speed_kmph / 24
            arrival = curr_time + tt
            if leg.is_pickup:
                departure = max(arrival, req.earliest_pickup)
                curr_load += 1
                ...
                if departure > req.latest_drop_hard + 1e-9:
                    return None
                curr_load -= 1
            if curr_load > vehicle.capacity or curr_load < 0:
                return None
            new_legs.append(RouteLeg(leg.request_id, leg.is_pickup, arrival, departure, curr_load))
            curr_time = departure
            prev_loc = curr_loc
    ...
    def destroy_route(self, solution, num_remove):
        new_sol = solution.shallow_copy()
        ranked = sorted([(vid, route.total_distance / max(len(route.request_set), 1),
    ...
    def repair_regret(self, solution, noise_level=0.0):
        ...
        for _ in range(len(pending) * 3):
            ...
            for _, req_id, ins in regrets:
                ...
                new_route = self._insert_request(new_sol.routes[vid].copy(),
                                                  self.requests[req_id], p_idx, d_idx)
                if new_route:
    ...
    def initial_solution(self):
        ...
        # Sort requests by priority and then by earliest pickup to prioritize
        # more constrained requests.
        sorted_reqs = sorted(self.requests_list,
                             key=lambda r: (r.priority, r.earliest_pickup))
        ...
        for req in sorted_reqs:
            ...
            # Candidates are vehicles that have previously served this request or are suitable
            candidate_vids = list(self.request_vehicle_map.get(req.id, []))
            # Add all other vehicles as well, as they might be able to serve it
            candidate_vids.extend([v.id for v in self.vehicles.values() if v.id not in candidate_vids])

            for vid in candidate_vids:
                if vid not in sol.routes:
                    continue

                current_route = sol.routes[vid]
                # Try appending the request at the end of the current route
                # This means inserting pickup and then dropoff at the end.
                # The _insert_request method can handle this by setting p_idx and d_idx
                # to the current length of legs and length + 1 respectively.
                p_idx = len(current_route.legs)
                d_idx = len(current_route.legs) + 1 # Dropoff will be after pickup

                temp_route = self._insert_request(current_route.copy(), req, p_idx, d_idx)

                if temp_route:
                    valid, _ = self._validate_route(temp_route)
                    if valid:
                        cost = self._calculate_route_cost(temp_route)
                        if cost < best_score:
                            best_score = cost
                            best_v = vid
                            best_route_after_insertion = temp_route

            if best_v:
                sol.routes[best_v] = best_route_after_insertion
                sol.unassigned.discard(req.id)
            # If not inserted, it remains in unassigned
        ...
        while iteration < max_iterations and time.time() - start_time < time_limit - 1:
            ...
            if config.DESTROY_RATE_ADAPTIVE:
                if self.stagnation_count > 500:
                    ...
                    self.current_destroy_min = max(config.DESTROY_RATE_MIN, self.current_destroy_min * 0.95)
                    self.current_destroy_max = max(config.DESTROY_RATE_MAX, self.current_destroy_max * 0.95)
            ...
            num_remove = max(1, int(len(self.requests) *
                                    random.uniform(self.current_destroy_min, self.current_destroy_max)))
            ...
            is_valid, _ = temp_sol.validate_solution(self.requests)
            if not is_valid:
                ...
                for i in range(self.n_destroy):
                    if self.destroy_usage[i] > 0:
                        self.destroy_weights[i] = max(
                            ...
                            (1 - config.WEIGHT_DECAY) * self.destroy_scores[i] / self.destroy_usage[i])
                        self.destroy_scores[i] = self.destroy_usage[i] = 0
                print(f"[ALNS] iter={iteration:04d} best={global_best_obj:.2f} "
                      f"served={self.global_best_solution.get_served_count()}/{len(self.requests_list)}")
            ...
            if iteration - last_improvement > config.EARLY_TERMINATION_ITERATIONS:
                print(f"[ALNS] Early stop: no improvement for {config.EARLY_TERMINATION_ITERATIONS} iters")
                break
            if iteration % 1000 == 0 and iteration > 0:
                if acceptance_count / 1000 < config.MIN_ACCEPTANCE_RATE:
                    print(f"[ALNS] Early stop: low acceptance rate")
                    break
                acceptance_count = 0
...
def time_to_fraction(value):
    ...
    if isinstance(value, (int, float)):
        # If value looks like minutes (e.g. 480 for 08:00), convert
        v = float(value)
...
def solve_alns(input_data, matrix_edge_list, file_bytes):
    """
    Standard interface called by algo/solver.py.
    Reads employees/vehicles from file_bytes (Excel), builds Location objects,
    runs EnhancedALNSSolver, and returns output in the route_sequence format.
    """
    ...
    for _, row in employees_df.iterrows():
        ...
        requests_list.append(Request(
            id=eid, priority=pri,
            pickup_loc=Location(plat, plng), drop_loc=Location(dlat, dlng),
    ...
    for _, row in vehicles_df.iterrows():
        ...
        vehicles_list.append(Vehicle(
            ...
            start_loc=Location(lat, lng), available_from=avail,
    ...
    for vid, route in best_sol.routes.items():
        ...
        for leg in route.legs:
            loc_id = leg.request_id if leg.is_pickup else "office"
            raw_seq.append({"location": loc_id,
    ...
    return {"vehicles": output_vehicles,
            "summary": {"total_cost_all_vehicles": round(total_cost_all, 2)}}

# ==========================================
# STANDALONE MAIN (run directly for testing)
# ==========================================
