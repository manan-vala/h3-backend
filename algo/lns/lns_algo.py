...
from lns_utils import load_data, Employee, Vehicle, haversine
...
d_EXCEL = "../TestCases/TestCase_TC12.xlsx" 
d_INITIAL = "../Output/input_lns.json"
d_OUTPUT = "../Output/output_lns.json"
class LNSOptimizer:
    def __init__(self, excel_path, initial_sol_path):
        self.employees, self.vehicles = load_data(excel_path)
        self.simulator = RouteSimulator(self.employees, self.vehicles, allow_violations=True)
    ...
    def destroy_worst(self, routes, num_remove):
        ...
        for v, groups in new_routes.items():
            for g in groups:
                for eid in g:
                    e = self.employees[eid]
                    dist = haversine(e.pickup_lat, e.pickup_lng, e.drop_lat, e.drop_lng)
    ...
    def optimize(self, max_iterations=100, destruction_rate=0.2):
        ...
        for i in range(max_iterations):
            ...
            if score < self.best_score:
                ...
                self.save("../Output/intermediate_best.json")
...
if __name__ == "__main__":
    ...
    INITIAL = d_INITIAL
    OUTPUT = d_OUTPUT
    lns = LNSOptimizer(EXCEL, INITIAL)
    lns.optimize(max_iterations=500, destruction_rate=0.4)
    lns.save(OUTPUT)
